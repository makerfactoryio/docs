...
#include <mySD.h> //  https://github.com/nhatuan84/esp32-micro-
#include <SPI.h>


#define SAMPLE_RATE 16000 //ESP-WROOM-32 is not able to sample a higher frequencies
...
unsigned int bitsPerSample = 24;   //confirmed
  ...
  if (digitalRead(btnStart) == LOW && recPressed == 0) 
  {
    ...
    while(1)
    {
      for (int i = 0; i < 513; i+=3)
      {
        ...
        byte_array[i] = ((data_sample & 0x0000ff00) >> 8) & 0xff;
        byte_array[i+1] = ((data_sample & 0x00ff0000) >> 16) & 0xff;
        byte_array[i+2] = ((data_sample & 0xff000000) >> 24) & 0xff; 
        recSamples++;
      }

      file.write(byte_array, 513);
