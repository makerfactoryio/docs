...
The LC network formed by L4 and C11 perform the impedance match from the PCB antenna to the 50 Ω required by the SoC. The capture below shows the return loss seen by the CC3210 RF pin. The VSWR is approximately 1.16:1 (-22 dB at the center of the WiFi band).
...
In order to route the RF signal to the u-Fl connector instead the PCB antenna, the 0 Ω resistor R5 needs to be unsoldered and installed in the R4 location.
